...
test("unsupported schema fails closed",()=>{const x=workspace();x.schemaVersion="9";const r=validatePlanningWorkspace(x);assert.equal(r.ok,false);assert.ok(r.diagnostics.some(d=>d.code==="UNSUPPORTED_SCHEMA_VERSION"));});
test("duplicate IDs fail closed",()=>{const x=workspace();x.areas.push({...x.areas[0]});const r=validatePlanningWorkspace(x);assert.equal(r.ok,false);assert.ok(r.diagnostics.some(d=>d.code==="DUPLICATE_OR_INVALID_AREA"));});
