...
test("machine and human checkpoint agree on admitted WO-008", () => {
  assert.equal(checkpoint.v11.status, "GBS_V11_WO_008_ADMITTED");
  ...
  assert.equal(checkpoint.v11.nextLegalAction, "CREATE_WO_008_IMPLEMENTATION_BRANCH_FROM_EXACT_ADMISSION_MERGE");
  assert.equal(checkpoint.v11.stopState, "GBS_V11_WO_008_ADMITTED_READY_FOR_IMPLEMENTATION_BRANCH");
  assert.ok(checkpointMd.includes("Active Work Order after this governance merge: `GBS-V11-WO-008`"));
  assert.ok(checkpointMd.includes("V1.1 STOP CONDITION: `GBS_V11_WO_008_ADMITTED_READY_FOR_IMPLEMENTATION_BRANCH`"));
});

test("WO-008 owns TELEM and enforces benchmark comparability", () => {
  ...
  assert.ok(wo.includes("STOP CONDITION: `GBS_V11_WO_008_READY_FOR_OBJECTIVE_AUDIT`"));
});
...
test("Direct Execution Brief freezes truthful telemetry boundaries", () => {
  ...
  assert.ok(brief.includes("GBS_V11_WO_008_READY_FOR_OBJECTIVE_AUDIT"));
});
