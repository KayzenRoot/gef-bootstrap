...
function fixture(options = {}) {
  ...
  const physical = {
    observe: async (path, context) => {
      contexts.push(context);
      return { ok: true, value: { ancestors: [{ relativePath: "", kind: "DIRECTORY", accessible: true, identityToken: "root-id", filesystemId: "fs1" }], target: { relativePath: path.normalizedRelativePath, kind: "FILE", accessible: true, identityToken: "id1", fingerprint: "before", filesystemId: "fs1", linkCount: 1 } } };
    },
    atomicFacts: async ({ path, context }) => {
      contexts.push(context);
      atomicCalls += 1;
      const drift = options.driftAt === atomicCalls;
      return { ok: true, value: { primitive: { capabilityRef: `cap:${path.operation}`, operation: path.operation, visibilityAtomic: true, raceResistant: true, noClobberCreate: false, replaceExisting: true, requiresSameFilesystem: true, durability: "UNPROVEN" }, stagingAuthorityRef: options.missingAuthority ? "" : drift ? "stage:changed" : "stage:original", stagingFilesystemId: drift && options.driftFilesystem ? "fs2" : "fs1", destinationFilesystemId: "fs1" } };
    },
  ...
  const adapter = createFilesystemEffectAdapter({ transactionId: "tx", resolver, physical, ...(options.signal === undefined ? {} : { signal: options.signal }), ...(options.deadlineMs === undefined ? {} : { deadlineMs: options.deadlineMs }), ...(options.nowMs === undefined ? {} : { nowMs: options.nowMs }) });
...
test("staging filesystem drift invalidates commit barrier instead of reusing old proof", async () => {
  ...
  assert.equal((await adapter.checkPhysicalSafety(intent)).ok, true);
  assert.equal((await adapter.revalidateCommitBarrier(plan)).ok, false);
});

test("physical observation and capability discovery receive the exact cancellation and deadline context", async () => {
  const controller = new AbortController();
  const nowMs = () => 100;
  const { adapter, contexts } = fixture({ signal: controller.signal, deadlineMs: 200, nowMs });
  assert.equal((await adapter.checkPhysicalSafety(intent)).ok, true);
  assert.ok(contexts.length >= 2);
  for (const context of contexts) {
    assert.equal(context.signal, controller.signal);
    assert.equal(context.deadlineMs, 200);
    assert.equal(context.nowMs, nowMs);
  }
});
