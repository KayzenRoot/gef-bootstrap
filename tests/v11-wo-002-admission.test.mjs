...
test('V1 production truth remains unchanged while WO-002 is admitted', () => {
...
test('V1.1 machine and human checkpoint agree on admitted WO-002', () => {
  assert.equal(checkpoint.v11.status, 'GBS_V11_WO_002_ADMITTED');
  ...
  assert.ok(checkpointMd.includes('Active Work Order after this admission merge: `GBS-V11-WO-002`'));
  assert.ok(checkpointMd.includes('V1.1 STOP CONDITION: `GBS_V11_WO_002_ADMITTED_READY_FOR_IMPLEMENTATION_BRANCH`'));
});

test('executor authority is effective but remains branch and Work-Order bounded', () => {
...
test('WO-002 is narrow and delegates future commands to their owning WOs', () => {
...
test('Context Lock binds the promoted release base and production immutables', () => {
...
test('Execution Brief is explicitly non-executable before admission merge and suppresses broad rediscovery', () => {
