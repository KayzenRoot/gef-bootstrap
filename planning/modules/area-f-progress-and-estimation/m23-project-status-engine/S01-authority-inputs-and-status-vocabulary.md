# GBS-M23-S01 — Authority, Inputs & Status Vocabulary
Status: `FROZEN`
Module weight: `13`
...
## Ownership boundary
M23 owns project lifecycle status, schedule-health interpretation, continuation-readiness interpretation, status transitions/revisions and status snapshots/handoffs. It does **not** own checkpoint promotion (M17), resume/re-entry truth (M18), progress calculation (M21), ETA/forecast calculation (M22), evidence/proof/assurance acceptance (M24/M25/M27), telemetry (M43), benchmark baselines (M45), executor performance (M63), or response rendering (M20/M47).

## Technologies
- **Status Intent Capsule (SIC23)**: immutable project/lineage request binding status horizon, explicit as-of value when schedule interpretation is requested, requested dimensions and exact upstream source identities.
...
2. M21 progress truth is required for `NOT_STARTED`, `IN_PROGRESS`, `AWAITING_ACCEPTANCE` or `COMPLETE` derivation.
...
5. Completion acceptance is an explicit owner-labeled input from an authorized canonical completion/assurance source; M21 `100%` alone never authorizes `COMPLETE`.
