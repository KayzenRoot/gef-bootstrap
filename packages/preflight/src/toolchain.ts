...
function validateDescriptor(descriptor: ToolDescriptor): string | null {
  ...
  if (descriptor.resolution.kind === "PATH_NAME") {
    return SAFE_PATH_NAME.test(descriptor.resolution.executable) ? null : "gef.preflight.tool.path_name_invalid";
  }
  ...
  return SAFE_POLICY_REF.test(descriptor.resolution.policyRef) ? null : "gef.preflight.tool.policy_ref_invalid";
}
...
export class ToolObservationSession {
  ...
  async #resolve(descriptor: ToolDescriptor): Promise<ToolResolutionResult> {
    ...
    try {
      const result = await this.#port.resolve(descriptor);
  ...
  async #probe(toolId: string, spec: ToolProbeSpec): Promise<ToolProbeResult> {
    ...
    try {
      result = await this.#port.probe(spec);
    } catch {
  ...
  async observe(request: ToolObservationRequest): Promise<ToolObservation> {
    ...
    if (probe.status !== "SUCCEEDED") {
      ...
      return { schemaVersion: 1, toolId: request.descriptor.toolId, presence: "FOUND", ...(resolution.executableIdentity ? { executableIdentity: resolution.executableIdentity } : {}), probeStatus: probe.status, compatibility: "UNKNOWN", gaps: [gap(code, required)] };
    }
    if (probe.exitCode !== 0 || !boundedText(probe.stdout, probeContract.maxOutputBytes) || !boundedText(probe.stderr, probeContract.maxOutputBytes)) {
      return { schemaVersion: 1, toolId: request.descriptor.toolId, presence: "FOUND", ...(resolution.executableIdentity ? { executableIdentity: resolution.executableIdentity } : {}), probeStatus: "FAILED", compatibility: "UNKNOWN", gaps: [gap("gef.preflight.tool.probe_output_invalid", required)] };
    }

    const parser = request.parseVersion ?? defaultVersionParser;
    const version = parser(probe.stdout, probe.stderr);
    if (!version || version.length > 128 || /[\u0000-\u001f\u007f]/.test(version)) {
      return { schemaVersion: 1, toolId: request.descriptor.toolId, presence: "FOUND", ...(resolution.executableIdentity ? { executableIdentity: resolution.executableIdentity } : {}), probeStatus: "SUCCEEDED", compatibility: "UNKNOWN", gaps: [gap("gef.preflight.tool.version_unparseable", required)] };
    }

    const compatibility = request.compatibility?.classify(request.descriptor.toolId, version) ?? "NOT_CHECKED";
