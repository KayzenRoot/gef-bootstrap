...
export function sessionFreezeCandidate(w:PlanningWorkspace,sessionId:string){const s=w.sessions.find(x=>x.sessionId===sessionId);if(!s)return {ready:false,diagnostics:Object.freeze(["SESSION_NOT_FOUND"])};const ds:string[]=[];if(s.openQuestionCount!==0)ds.push("OPEN_QUESTIONS");if(!s.stopCondition)ds.push("MISSING_STOP_CONDITION");for(const id of s.dependencyIds)if(!w.dependencies.some(d=>d.dependencyId===id)&&![...w.modules,...w.sessions].some(x=>("moduleId" in x?x.moduleId:x.sessionId)===id))ds.push("UNKNOWN_DEPENDENCY");if(s.status==="STALE"||s.status==="BLOCKED"||s.status==="SUPERSEDED")ds.push("NON_FREEZABLE_STATUS");return {ready:ds.length===0,diagnostics:Object.freeze(ds)};}
