import { asciiFoldTarget, checkControl, fail, freezeBytes, RENDER_CONTRACT_VERSION, stableStringify, VALIDATION_CONTRACT_VERSION, validateLogicalFileTarget } from "./runtime.js";
...
function validationSnapshot(render: RenderSnapshot, digest: DigestPort, outcome: TemplateValidationSnapshot["outcome"], artifacts: readonly DesiredArtifact[], gaps: readonly string[]): TemplateValidationSnapshot {
  ...
  const templateValidationDigest = digest.digest(stableStringify({ renderSnapshotDigest: render.renderSnapshotDigest, validationContractVersion: VALIDATION_CONTRACT_VERSION, outcome, artifacts: identity, gaps }));
  return Object.freeze({ validationContractVersion: VALIDATION_CONTRACT_VERSION, outcome, renderSnapshotDigest: render.renderSnapshotDigest, artifacts: Object.freeze([...artifacts]), downstreamRequirements: Object.freeze(["M05_CURRENT_STATE_CLASSIFICATION", "M06_PHYSICAL_PATH_AUTHORITY"]), templateValidationDigest, gaps: Object.freeze([...gaps]) });
}
...
function collisionCode(desired: readonly DesiredArtifact[]): string | null {
  const exact = new Map<string, string>();
  const folded = new Map<string, string>();
  for (const item of desired) {
    if (exact.has(item.logicalTarget)) return "TARGET_DUPLICATE";
    if (folded.has(item.targetCollisionKey)) return "TARGET_CASE_ALIAS_COLLISION";
  ...
  for (const item of desired) {
    ...
    for (let index = 1; index < parts.length; index += 1) {
      ...
      if (exact.has(ancestor) || folded.has(asciiFoldTarget(ancestor))) return "TARGET_ANCESTOR_DESCENDANT_COLLISION";
    }
  }
  return null;
}
...
export function validateTemplate(template: TemplateDescriptor, render: RenderSnapshot, digest: DigestPort, control: TemplateControl): TemplateResult<TemplateValidationSnapshot> {
  ...
  if (!template || !render || render.renderContractVersion !== RENDER_CONTRACT_VERSION || template.templateSemanticDigest !== render.templateSemanticDigest || !Array.isArray(template.entries) || !Array.isArray(render.artifacts) || !Array.isArray(render.gaps)) return fail("VALIDATION_INPUT_MISMATCH", "VALIDATION", "Template and render snapshots are incompatible.");
  if (render.artifacts.length > control.budgets.maxEvidenceEntries) return blocked(render, digest, "VALIDATION_EVIDENCE_BUDGET_EXCEEDED");
  ...
  for (const artifact of render.artifacts) {
    ...
    if (artifact.content.length > control.budgets.maxRenderedBytesPerEntry) return blocked(render, digest, "RENDER_CONTENT_BUDGET_EXCEEDED");
    validatedBytes += artifact.content.length;
    if (validatedBytes > control.budgets.maxRenderedBytesTotal) return blocked(render, digest, "RENDER_CONTENT_TOTAL_BUDGET_EXCEEDED");
  ...
  const expectedRenderDigest = digest.digest(stableStringify({ renderContractVersion: render.renderContractVersion, templateSemanticDigest: render.templateSemanticDigest, variableValueDigest: render.variableValueDigest, conditionalDecisionDigest: render.conditionalDecisionDigest, artifacts: renderIdentity }));
  if (expectedRenderDigest !== render.renderSnapshotDigest) return blocked(render, digest, "RENDER_SNAPSHOT_DIGEST_MISMATCH");

  const collision = collisionCode(desired);
  if (collision) return blocked(render, digest, collision);

  if (render.gaps.length > 0) return { ok: true, value: validationSnapshot(render, digest, "INDETERMINATE", [], ["RENDER_MANDATORY_GAP"]) };
  if (render.artifacts.length !== template.entries.length || seenEntries.size !== template.entries.length) return blocked(render, digest, "ARTIFACT_ACCOUNTING_MISMATCH");
