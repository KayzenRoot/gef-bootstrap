...
const stable=v=>JSON.stringify(v,(_,x)=>x&&typeof x==='object'&&!Array.isArray(x)?Object.fromEntries(Object.entries(x).sort(([a],[b])=>a<b?-1:a>b?1:0)):x);
const lexical=(a,b)=>a<b?-1:a>b?1:0;
export const digest=(d,v)=>createHash('sha256').update(`${d}\0${stable(v)}`).digest('hex');
export function helpIndex(commands){return [...commands].sort((a,b)=>lexical(a.id,b.id)).map(x=>({id:x.id,summary:x.summary,schema:x.schema??null}));}
