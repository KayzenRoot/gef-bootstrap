// M15 Execution Pack Compiler - utils.ts
// Shared deterministic utilities. No side effects.
// Mirrors the task-context-compiler utility contract so executor behavior stays uniform.
...
/** Compare two code-point-ordered string arrays lexicographically. */
export function compareStringArrays(a: readonly string[], b: readonly string[]): number {
  const limit = Math.min(a.length, b.length);
  for (let i = 0; i < limit; i++) {
    const left = a[i];
    const right = b[i];
    if (left !== right) return compareCodePoint(left ?? '', right ?? '');
  }
