// M14 Task & Context Compiler - utils.ts
...
export const DEFAULT_MAX_UNITS = 512;
export const DEFAULT_MAX_DOMAINS = 128;
...
/** Sort-deduplicate a string array. Returns error on duplicate. */
export function uniqueSorted(values: readonly string[]): Result<readonly string[]> {
  ...
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i] === sorted[i - 1]) {
      return fail('DUPLICATE_VALUE', 'Duplicate semantic value', sorted[i]);
    }
  }
...
export function mergeFailures<T>(results: ReadonlyArray<Result<unknown>>): Result<T> | null {
  const failed = results.filter(r => !r.ok) as Array<{ ok: false; diagnostics: readonly Diagnostic[] }>;
  if (failed.length === 0) return null;
  return { ok: false, diagnostics: failed.flatMap(f => f.diagnostics) };
}
