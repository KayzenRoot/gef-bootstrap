...
export async function rollbackTransaction(input: RollbackTransactionInput): Promise<RollbackReceipt> {
  if (!verifyTransactionPlanDigest(input.plan, input.ports.digest) || input.applyReceipt.planDigest !== input.plan.planDigest) return receipt(input, [], "RECOVERY_ESCALATION_REQUIRED");
  ...
  for (let index = 0; index < applied.length; index += 1) {
    ...
    if (input.signal?.aborted === true) {
      for (let remaining = index; remaining < applied.length; remaining += 1) {
        const unresolved = applied[remaining];
        if (unresolved !== undefined) results.push(Object.freeze({ intentId: unresolved.intentId, targetRef: unresolved.targetRef, outcome: "FAILED" }));
      }
      break;
    }
