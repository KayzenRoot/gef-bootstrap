...
export interface AuthorityBindingRef {
...
export interface CheckpointStateEntry {
  readonly domain: string;
  readonly status: ContinuationStatus;
...
export interface CanonicalContinuationCapsuleInput {
  ...
  readonly predecessorCheckpointDigest: string | null;
  readonly admittedWorkOrderIds: readonly string[];
  readonly authorityBindings: readonly AuthorityBindingRef[];
  readonly policyBinding: ContinuityPolicyBinding;
  readonly claims: readonly ContinuationClaim[];
...
export interface CanonicalContinuationCapsule extends CanonicalContinuationCapsuleInput {
  readonly stateVector: CheckpointStateVector;
  readonly authorityIndex: AuthoritySnapshotIndex;
  readonly checkpointDigest: string;
}

export interface ContinuationInvariant {
  readonly invariantId: string;
  readonly satisfied: boolean;
  readonly reason: string;
}
export interface ContinuationInvariantSet {
  readonly invariants: readonly ContinuationInvariant[];
  readonly admissible: boolean;
  readonly invariantDigest: string;
}

export interface PromotionFenceToken {
...
export interface CheckpointPromotionProposal {
  readonly expectedBaseDigest: string;
  readonly candidate: CanonicalContinuationCapsule;
  readonly fenceToken: PromotionFenceToken;
  readonly authorizationRef: string;
  readonly proposalDigest: string;
}

export interface SemanticCasResult {
  readonly status: 'MATCH' | 'STALE_BASE';
  readonly expectedBaseDigest: string;
  readonly currentBaseDigest: string;
}

export interface SplitBrainFinding {
  readonly baseCheckpointDigest: string;
  readonly successorDigests: readonly string[];
  readonly divergent: boolean;
}

export interface CheckpointMutationReceipt {
  readonly status: PromotionStatus;
  readonly beforeCheckpointDigest: string;
  readonly afterCheckpointDigest: string;
  readonly lineageId: string;
...
export interface InvalidationResult {
  ...
  readonly conservativeWidening: boolean;
}

export interface CheckpointRollbackPointer {
  readonly fromCheckpointDigest: string;
...
export type ContinuityRegressionKind = 'MATURITY_ROLLBACK' | 'DONE_TO_NON_DONE' | 'AUTHORITY_LOSS' | 'WORK_ORDER_LOSS' | 'NEXT_ACTION_AMBIGUITY';
...
export interface HistoricalPointer {
  readonly checkpointDigest: string;
  readonly relation: 'PREDECESSOR' | 'SUPERSEDED' | 'ROLLBACK_SOURCE' | 'EVIDENCE';
}
...
export interface CheckpointPortabilityEnvelope {
  readonly schemaVersion: 1;
  readonly projectId: string;
  readonly lineageId: string;
  readonly checkpointDigest: string;
...
export interface CheckpointSizeAssessment {
...
export interface CheckpointAdmissionReceipt {
  readonly validity: CheckpointValidity;
  readonly beforeCheckpointDigest: string;
  readonly afterCheckpointDigest: string;
  readonly lineageId: string;
...
export interface ResumeReadinessCertificate {
  readonly validity: CheckpointValidity;
  readonly checkpointDigest: string;
  readonly ready: boolean;
  readonly missingBindingIds: readonly string[];
  readonly unresolvedBlockers: readonly string[];
  readonly requiredCapabilityGaps: readonly string[];
...
export interface ContinuationHandoffContract {
  readonly checkpointDigest: string;
  readonly projectId: string;
  readonly lineageId: string;
  readonly nextLegalAction: string;
