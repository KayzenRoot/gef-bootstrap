...
function verifyQuarantine(quarantine:ResumeConflictQuarantine,options:OperationOptions):Result<true>{const d=sha(options,quarantine.entries);if(!d.ok)return d as Result<true>;return d.value===quarantine.quarantineDigest?{ok:true,value:true}:fail('RESUME_QUARANTINE_TAMPERED','Resume conflict quarantine digest does not match its payload');}
...
export function buildResumeReceipt(intent:ResumeIntentCapsule,checkpoint:CanonicalContinuationCapsule,handoff:ContinuationHandoffContract,decision:SafeReentryDecision,readPlan:ResumeReadPlan,drift:ResumeDriftVector,orphans:OrphanWorkReport,quarantine:ResumeConflictQuarantine,options:OperationOptions):Result<ResumeReceipt>{
  const c=cancelled(options);if(c)return c;const iv=verifyResumeIntentCapsule(intent,options);if(!iv.ok)return iv as Result<ResumeReceipt>;const cv=verifyCanonicalContinuationCapsule(checkpoint,{digest:options.digest,cancellation:options.cancellation});if(!cv.ok)return{ok:false,diagnostics:cv.diagnostics};const hv=verifyContinuationHandoff(handoff,options);if(!hv.ok)return hv as Result<ResumeReceipt>;const vd=verifySafeReentryDecision(decision,options);if(!vd.ok)return vd as Result<ResumeReceipt>;const vp=verifyReadPlan(readPlan,options);if(!vp.ok)return vp as Result<ResumeReceipt>;const vv=verifyDrift(drift,options);if(!vv.ok)return vv as Result<ResumeReceipt>;const vo=verifyOrphans(orphans,options);if(!vo.ok)return vo as Result<ResumeReceipt>;const vq=verifyQuarantine(quarantine,options);if(!vq.ok)return vq as Result<ResumeReceipt>;
  if(decision.checkpointDigest!==checkpoint.checkpointDigest||handoff.checkpointDigest!==checkpoint.checkpointDigest||decision.intentDigest!==intent.intentDigest||decision.handoffDigest!==handoff.handoffDigest)return fail('RESUME_RECEIPT_BINDING_MISMATCH','Resume receipt inputs are not bound to one intent, handoff and checkpoint');
...
export function detectContinuityLoss(previous:CanonicalContinuationCapsule,current:CanonicalContinuationCapsule,currentReceipt:ResumeReceipt,options:OperationOptions):Result<readonly ContinuityLossFinding[]>{
  const c=cancelled(options);if(c)return c;const pv=verifyCanonicalContinuationCapsule(previous,{digest:options.digest,cancellation:options.cancellation});if(!pv.ok)return{ok:false,diagnostics:pv.diagnostics};const cv=verifyCanonicalContinuationCapsule(current,{digest:options.digest,cancellation:options.cancellation});if(!cv.ok)return{ok:false,diagnostics:cv.diagnostics};const rv=verifyResumeReceipt(currentReceipt,options);if(!rv.ok)return rv as Result<readonly ContinuityLossFinding[]>;const findings:ContinuityLossFinding[]=[];
...
export function buildResumeEfficiencyReceipt(plannedReads:number,hotHits:number,negativeCacheHits:number,expansionReads:number,options:OperationOptions):Result<ResumeEfficiencyReceipt>{
  ...
  if(negativeCacheHits>plannedReads)return fail('RESUME_EFFICIENCY_INVALID','Negative-cache hits cannot exceed planned reads');
...
export function buildSafeHandbackContract(decision:SafeReentryDecision,receipt:ResumeReceipt,options:OperationOptions):Result<SafeHandbackContract>{
  const vd=verifySafeReentryDecision(decision,options);if(!vd.ok)return vd as Result<SafeHandbackContract>;const verified=verifyResumeReceipt(receipt,options);if(!verified.ok)return verified as Result<SafeHandbackContract>;
  if(receipt.status!==decision.status||receipt.checkpointDigest!==decision.checkpointDigest||receipt.decisionDigest!==decision.decisionDigest||receipt.handoffDigest!==decision.handoffDigest||receipt.nextAction!==decision.nextAction)return fail('HANDBACK_RECEIPT_MISMATCH','Handback decision and receipt are not bound to the same resume result');
