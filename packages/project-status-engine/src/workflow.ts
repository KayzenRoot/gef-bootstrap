import type{EvaluateProjectStatusOutput,OperationOptions,ProjectStatusInput,ProjectStatusSnapshot,Result}from'./types.js';
...
export function evaluateProjectStatus(input:ProjectStatusInput,predecessor:ProjectStatusSnapshot|null,options:OperationOptions):Result<EvaluateProjectStatusOutput>{
 ...
 const snapshot=createProjectStatusSnapshot(input,predecessor,options);if(!snapshot.ok)return snapshot;
