...
export function validId(value: string): boolean { return ID_RE.test(value); }
export function safeLimit(value: number | undefined, fallback: number): number { return Math.max(1, Math.min(value ?? fallback, fallback)); }
...
export function sha(options: OperationOptions, value: unknown): Result<string> {
  ...
  try {
    ...
    const code = error instanceof Error && error.message === 'CANONICAL_CYCLE' ? 'INPUT_CYCLE' : error instanceof Error && error.message === 'UNSUPPORTED_PROTOTYPE' ? 'UNSUPPORTED_PROTOTYPE' : 'DIGEST_CAPABILITY_FAILURE';
...
export function uniqueSorted(values: readonly string[]): Result<readonly string[]> {
  const sorted = [...values].sort(compareCodePoint);
  for (let i = 1; i < sorted.length; i++) if (sorted[i] === sorted[i - 1]) return fail('DUPLICATE_VALUE', 'Duplicate semantic value', sorted[i]);
  return { ok: true, value: sorted };
}

export function hasSecretLikeKey(value: unknown, seen = new Set<object>()): boolean {
  if (!value || typeof value !== 'object') return false;
  const objectValue = value as object;
  if (seen.has(objectValue)) return false;
  seen.add(objectValue);
  try {
    if (Array.isArray(value)) return value.some(v => hasSecretLikeKey(v, seen));
    for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
      if (/(?:password|secret|credential|token|api[-_]?key)/i.test(key)) return true;
      if (hasSecretLikeKey(child, seen)) return true;
    }
    return false;
  } finally { seen.delete(objectValue); }
}
