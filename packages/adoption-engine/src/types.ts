...
export type TruthResolutionState = 'ALIGNED' | 'UNRESOLVED' | 'LEGACY_ACCEPTED' | 'BLOCKED' | 'RESOLUTION_ADMITTED';
...
export interface TemplateBinding { id:string; version:string; digest:string; }
export interface SeedNode { id:string; semanticClass:string; sourceOwner:string; creationMode:'CREATE_FROM_EXPLICIT_INPUT'|'CREATE_SKELETON_REQUIRING_DECISION'|'EXPLICIT_NOT_APPLICABLE'; dependencies?:readonly string[]|undefined; decisionRefs?:readonly string[]|undefined; profileIdentity:string; profileDigest:string; templateRef?:Readonly<TemplateBinding>|undefined; mutationOwner:string; postconditionChecks:readonly string[]; }
...
export interface ProvenanceRecord { artifactId:string; sourceRefs:readonly string[]; templateRef?:Readonly<TemplateBinding>|undefined; semanticOwner:string; generatedDigest:string; applyReceiptRef?:string|undefined; validationState:'VALID'|'INCOMPLETE'; }
export interface TruthPair { domain:string; observedTruth:unknown; normativeTruth?:unknown; bindingEvidence:readonly string[]; driftClass:DriftClass; confidence?:number|undefined; resolutionState:TruthResolutionState; resolutionRef?:string|undefined; legacyAcceptanceRef?:string|undefined; }
export interface LegacyCompatibilityMembrane { id:string; legacySourceIdentity:string; semanticClass:string; mappingType:LegacyMappingType; lossy:boolean; approvalRef?:string|undefined; admissionRef?:string|undefined; sourceFingerprint:string; owner:string; mutationPermission?:'NONE'|undefined; }
export interface CompatibilityBridgeContract { id:string; sourceIdentity:string; targetSemanticClass:string; mappingVersion:string; direction:BridgeDirection; lossy:boolean; approvalRef?:string|undefined; conflictBehavior:'FAIL'|'SURFACE_DRIFT'; invalidationFingerprint:string; owner:string; reviewTrigger:string; }
export interface AdoptionDependency { domain:string; dependsOn?:readonly string[]|undefined; }
export interface GovernanceMaturityDelta { domain:string; from:GovernanceMaturity; to:GovernanceMaturity; }
export interface AdoptionSlice { targetDomains:readonly string[]; requiredDomains:readonly string[]; requiredMappings:readonly string[]; unresolvedBlockers:readonly string[]; blockedDomains:readonly string[]; optionalCleanup:readonly string[]; postAdoptionMaturityDelta:readonly GovernanceMaturityDelta[]; safetyEnvelopeIdentity:string; }
...
export interface LegacyDebtRecord { id:string; domain:string; sourceBinding:string; driftClass:DriftClass; severity:'LOW'|'MEDIUM'|'HIGH'|'CRITICAL'; reason:string; affectedCapabilities:readonly string[]; promotionBlocker:boolean; invalidationCondition:string; }
...
export interface GovernanceDeltaReceipt { projectId:string; beforeIdentity:string; afterIdentity:string; frontierMoved:readonly string[]; bridgesAdded:readonly string[]; bridgesRetired:readonly string[]; driftChanges:readonly string[]; quarantineChanges:readonly string[]; capabilitiesUnlocked:readonly string[]; capabilitiesInvalidated:readonly string[]; evidenceBindings:readonly string[]; semanticIdentity:string; }
export interface SourceFingerprintBinding { id:string; fingerprint:string; }
export interface DriftSummaryEntry { domain:string; driftClass:DriftClass; severity:'LOW'|'MEDIUM'|'HIGH'|'CRITICAL'; }
export interface QuarantineSummaryEntry { id:string; domain:string; severity:'LOW'|'MEDIUM'|'HIGH'|'CRITICAL'; promotionBlocker:boolean; }
export interface AdoptionReceiptInput { projectId:string; policyVersion:string; mode:AdoptionMode; intentCapsuleIdentity:string; sourcePackIdentity:string; profileIdentity:string; profileDigest:string; maturityVectorIdentity:string; maturityVector:GovernanceMaturityVector; normalizationFrontier:NormalizationFrontier; bridgeIds:readonly string[]; membraneIds:readonly string[]; sourceFingerprints:readonly SourceFingerprintBinding[]; driftSummary:readonly DriftSummaryEntry[]; quarantineSummary:readonly QuarantineSummaryEntry[]; completedIncrementIds:readonly string[]; blockers:readonly string[]; safetyEnvelopeIdentity:string; capabilityUnlocks:readonly CapabilityUnlock[]; governedDomains:readonly string[]; ungovernedDomains:readonly string[]; blockedDomains:readonly string[]; nextSafeSlice?:AdoptionSlice|undefined; }
...
export interface AdoptionRegression { code:string; domain?:string|undefined; capability?:string|undefined; bindingId?:string|undefined; beforeMaturity?:GovernanceMaturity|undefined; afterMaturity?:GovernanceMaturity|undefined; message:string; }
