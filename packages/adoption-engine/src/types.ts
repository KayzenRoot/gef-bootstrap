...
export interface AdoptionIntentInput {
  projectId: string;
  mode: AdoptionMode;
...
export interface AdoptionSafetyEnvelope {
  allowedMutationSurfaces: readonly string[];
  forbiddenSurfaces: readonly string[];
  reversibleOperations: readonly string[];
  preconditions: readonly string[];
  requiredChecks: readonly string[];
  abortConditions: readonly string[];
  rollbackOwner: string;
  maxUnresolvedSeverity: 'LOW' | 'MEDIUM' | 'HIGH';
}

export interface NewProjectSignals {
  explicitNewProjectIntent: boolean;
  conflictingCanonicalSystem: boolean;
  retainedHistory: boolean;
  existingReleaseEvidence: boolean;
  remoteProjectEvidence: boolean;
}
...
export interface LegacyCompatibilityMembrane { id: string; legacySourceIdentity: string; semanticClass: string; mappingType: LegacyMappingType; lossy: boolean; approvalRef?: string | undefined; sourceFingerprint: string; owner: string; }
export interface CompatibilityBridgeContract { id: string; sourceIdentity: string; targetSemanticClass: string; mappingVersion: string; direction: BridgeDirection; lossy: boolean; approvalRef?: string | undefined; conflictBehavior: 'FAIL' | 'SURFACE_DRIFT'; invalidationFingerprint: string; owner: string; }
...
export interface LegacyDebtRecord { id: string; domain: string; sourceBinding: string; driftClass: DriftClass; severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'; reason: string; affectedCapabilities: readonly string[]; promotionBlocker: boolean; }
...
export interface AdoptionReceiptInput {
  projectId: string;
  policyVersion: string;
  mode: AdoptionMode;
  intentCapsuleIdentity: string;
  sourcePackIdentity: string;
  profileIdentity: string;
  profileDigest: string;
  maturityVectorIdentity: string;
  normalizationFrontier: NormalizationFrontier;
  bridgeIds: readonly string[];
  membraneIds: readonly string[];
  driftSummary: readonly string[];
  quarantineIds: readonly string[];
  completedIncrementIds: readonly string[];
  blockers: readonly string[];
  safetyEnvelopeIdentity: string;
  capabilityUnlocks: readonly CapabilityUnlock[];
}
...
export interface ReceiptCurrentBindings { projectId: string; sourcePackIdentity: string; policyVersion: string; }
export interface AdoptionRegression { code: string; domain?: string | undefined; capability?: string | undefined; message: string; }
