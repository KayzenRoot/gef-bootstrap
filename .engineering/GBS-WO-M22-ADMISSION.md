...
## Admission scope
Admission may authorize only the bounded implementation of the 30 M22 mechanisms frozen in S01-S05 and compiled into `GBS-WO-M22-001`.
...
## ELEVATED admission obligations
...
## Execution-base rule
This candidate does not yet create implementation authority. If this admission PR passes exact-head semantic audit and merges, a separate post-merge binding must record that actual merge SHA as the sole legal M22 execution base and promote Work Order/checkpoint to `ADMITTED_READY_FOR_IMPLEMENTATION`.

## Credit rule
M22 remains `0 / 15`. Admission grants execution authority only after the post-merge binding. Production credit requires implementation, ELEVATED evidence, exact-head semantic review, implementation merge and separate MODULE_DONE promotion.

STOP CONDITION: `GBS_WO_M22_001_ADMISSION_CANDIDATE_PENDING_AUDIT`.
